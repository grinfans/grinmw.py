...
def mocked_get(m, target_url, mocked_response, status_code=200):
    m.get(target_url, json=mocked_response, status_code=status_code)
...
def mock_post(
    ...
    m.post(
        ...
        additional_matcher=(
            matcher
            if (expected_body is not None or expected_auth is not None)
            else None
        ),
    )
...
def mock_post_key_exchange(
    ...
    def response(request):
        return {"jsonrpc": "2.0", "id": 1, "result": {"Ok": mock_response_pubkey_hex}}
...
def mock_post_encrypted(
    ...
    def response_callback(request, context):
        ...
        return {
            "result": {"Ok": {"nonce": resp_nonce.hex(), "body_enc": encrypted_resp}}
        }

    # Register the mock with matcher + dynamic response
    m.post(target_url, additional_matcher=custom_matcher, json=response_callback)
